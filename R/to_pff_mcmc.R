...
to_pff_mcmc <- function(mcmc) {
  ...
  if (!peregrine::is_pff(mcmc$treelog$filename)) { # nolint peregrine function
    mcmc$treelog$filenames <- peregrine::to_pff(mcmc$treelog$filename) # nolint peregrine function
  }
